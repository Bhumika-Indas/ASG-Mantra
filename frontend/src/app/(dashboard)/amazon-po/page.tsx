...
export default function AmazonPOPage() {
  ...
  const handleStatusChange = async (poItem: POItem, newDisplayStatus: string) => {
    ...
    try {
      ...
      await api.purchaseOrders.updateAmazonItemStatus(poItem.id, { status: dbStatus });
  ...
  useEffect(() => {
    const fetchAmazonPOs = async () => {
      try {
        ...
        // Fetch Amazon purchase orders
        const response = await api.purchaseOrders.getAmazon() as any;
  ...
  const filteredPoData = useMemo(() => {
    ...
    // Channel filter (always amazon for this page, but included for consistency)
